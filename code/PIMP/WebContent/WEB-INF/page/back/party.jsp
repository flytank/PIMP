...
<script type="text/javascript">
	$(function(){
		...
		$("#query").on("click", function() {
			...
			//党员信息
			jsonData += "{'field' : 'identity', 'op' : 'eq', 'data' : '1'}";
		...
		$("#reset").on("click", function() {
			...
			//党员信息
			var jsonData = "{'rules':[{'field' : 'identity', 'op' : 'eq', 'data' : '1'}], 'groupOp':'AND'}";
			$("#grid-table").jqGrid('setGridParam',{
		        datatype:'json',
		        postData:{'filters':jsonData}, //发送数据
		        page:1
...
<input type="hidden" id="loginUser" name="loginUser" value="${sessionScope.SESSION_SYS_USER}">
<div class="row">
	<div class="col-xs-12">
					...
					<div class="widget-box widget-color-blue2">
						<div class="widget-body">
							<div class="widget-main padding-8">
							   <form id="validationAddPartyForm" method="post" action="#">
								<table id="information" class="table table-striped table-bordered table-hover">
        			            	<tr>
        			            	<td class='hidden-480' width='110px'>登录名</td>
									<td>
										<input type='text' id='selectLoginName' name='selectLoginName' size='40' class='form-control limited' maxlength='40' />
									</td>
									</tr>
									<tr>
        			            	<td class='hidden-480' width='110px'>姓名</td>
									<td>
										<input type='text' id='selectUserName' name='selectUserName' size='40' class='form-control limited' maxlength='50' />
									</td>
									</tr>
									<tr>
        			            	<td class='hidden-480' width='110px'>身份证</td>
									<td>
										<input type='text' id='selectIDCard' name='selectIDCard' size='40' class='form-control limited' maxlength='18'>
									</td>
									</tr>
									<tr>
									...
									<td>
										<input id='selectBirthday' name='selectBirthday' size='40' class="cond-input-date" />
									</td>
									</tr>
									<tr>
									...
									<td>
										<input type='text' id='selectPosition' name='selectPosition' size='40' class='form-control limited' maxlength='50'>
									</td>
									</tr>
									<!-- <tr>
									<td class='hidden-480' width='110px' style="color:red;">身份</td>
									<td>
									...
									<td>
										<select id='selectTeamID' name='selectTeamID' disabled></select>
									</td>
									</tr>
									<tr>
									<td class='hidden-480' width='110px'>党员类别</td>
									<td>
										<select id='selectSortID' name='selectSortID'></select>
									</td>
									</tr>
									<tr>
									<td class='hidden-480' width='110px'>角色</td>
									<td>
										<select id='selectRole' name='selectRole'></select>
									</td>
									</tr>
									<tr>
			...
			$("#selectTownID").ready(function() {
				$.ajax({
					...
					success : function(data) {
						...
						if(role==roles.town||role==roles.village){
							$("#selectTownID").val(townID);
							$("#queryTownID").val(townID);
							$("#queryTownID").prop("disabled",true);
							$("#selectTownID").prop("disabled",true);
							$("#queryVillageID").prop("disabled",false);
							$("#selectVillageID").prop("disabled",false);
						}
...
//									document.getElementById("selectVillageID").disabled = true;
								if(role==roles.village){
									$("#selectVillageID").val(villageID);
									$("#selectVillageID").prop("disabled",true);
									$("#queryVillageID").val(villageID);
									$("#queryVillageID").prop("disabled",true);
									document.getElementById("selectTeamID").disabled = false;//党小组不可操作
									$("#selectTeamID").ready(function(){
										var selectTownID = $("#selectTownID").val();
				...
				$("#selectTownID").change(function() {
					...
					var selectVillageID = $("#selectVillageID").val();
					if('' == selectTownID || '' == selectVillageID){//身份不是党员或者入党积极分子
						$("#selectTeamID").empty();
...
// 								}
								//var selectIdentity = $("#selectIdentity").val();
								if('' == selectTownID ){//身份不是党员或者入党积极分子
									document.getElementById("selectTeamID").disabled = true;//党小组不可操作
								}else{
									document.getElementById("selectTeamID").disabled = false;//党小组可操作
								}
						...
						$("#selectTeamID").empty();
						document.getElementById("selectTeamID").disabled = true;//党小组可操作
					}
				})
			})
// 			if(role==roles.town||role==roles.village){
...
// 			}, "此登录名不唯一！");
		...
		$('#validationAddPartyForm').validate({
			errorElement : 'span',
        ...
        $('.page-content-area').ace_ajax('loadScripts', scripts, function() {
        	...
        	jQuery(function($) {
        		...
        		jQuery(grid_selector).jqGrid({
        			subGrid : false,
        			url : "${contextPath}/sys/sysuser/getParty",
        			datatype : "json",
        			postData:{'filters':jsonData}, //发送数据
        			height : 450,
        			...
        			*/
        			gridComplete : function(){
        				...
        				for(var i = 0; i < ids.length; i++){
        					...
        					var role = '${sessionScope.SESSION_SYS_USER.role}';
        					if(role == 'ROLE_ADMIN' || role == 'zone'){//超级管理员&&区域管理员
            					//look = "<a href='#page/party' onclick='LookData(\"" + id + "\")'>修改</a>&nbsp;&nbsp;&nbsp;"
            					//	+"<a href='#page/party' onclick='resetPassword(\"" + id + "\")'>重置密码</a>";	
            					look = "<a href='#page/party' onclick='LookData(\"" + id + "\")'>修改</a>&nbsp;&nbsp;&nbsp;"
            						+"<a href='#page/party' onclick='resetPassword(\"" + id + "\")'>重置密码</a>";	
            					jQuery(grid_selector).jqGrid('setRowData', ids[i], {HANDLE : look});
        					}else if(role == 'town'){//乡镇管理员
        						if(rowData.roleCn == '超级管理员' 
        								|| rowData.roleCn == '区域管理员'){
        					...
        					}else if(role == 'village'){//村级管理员
        						if(rowData.roleCn == '超级管理员' 
    								|| rowData.roleCn == '区域管理员'
    								|| rowData.roleCn == '乡镇管理员'){
        						}else{
    							look = "<a href='#page/party' onclick='LookData(\"" + id + "\")'>修改</a>&nbsp;&nbsp;&nbsp;"
        						+"<a href='#page/party' onclick='resetPassword(\"" + id + "\")'>重置密码</a>";
            					jQuery(grid_selector).jqGrid('setRowData', ids[i], {HANDLE : look});
